...
In the previous chapter, you've seen how to use JavaScript on the server to dynamically generate multiple pages of HTML. For most simple websites, that's all you need. If you want it to look fancy, invest in learning more about design and CSS.
...
Mastro comes with its own minimal take on a client-side rendering library: [Reactive Mastro](/reactive/). Like many other reactive libraries (but unlike React), it uses _signals_ to hold state.
